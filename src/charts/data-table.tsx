...
import type { DataTableProps, ColumnConfig } from '../lib/types';
import { autoFormat, applyFormat, parseFmtProp } from '../lib/format-value';
import { formatColumnHeader } from '../lib/chart-utils';

/** Build a column→format map from ColumnConfig array */
function buildFmtMapFromConfigs(configs: ColumnConfig[]): Map<string, string> | null {
  const map = new Map<string, string>();
  for (const c of configs) {
    if (c.fmt) map.set(c.field, c.fmt);
  }
...
export function DataTable({ data, columns: explicitColumns, fmt, columnConfigs, rows: rowsProp, queryLimit }: DataTableProps) {
  const [sort, setSort] = useState<SortState>(null);
  const [page, setPage] = useState(0);

  const sortedData = useMemo(() => {
    if (!sort || data.length === 0) return data;
    return [...data].sort((a, b) => compareValues(a[sort.col], b[sort.col], sort.asc));
  }, [data, sort]);

  if (data.length === 0) return null;

  // columnConfigs takes precedence over columns/fmt props
  const columns = columnConfigs
    ? columnConfigs.map((c) => c.field)
    : explicitColumns ?? Object.keys(data[0]);
  const fmtMap = columnConfigs
    ? buildFmtMapFromConfigs(columnConfigs)
    : fmt ? parseFmtProp(fmt) : null;
  const headerMap = columnConfigs
    ? new Map(columnConfigs.filter((c) => c.header).map((c) => [c.field, c.header!]))
    : null;
                ...
                return (
                  ...
                  >
                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, flexDirection: isNumeric ? 'row-reverse' : 'row' }}>
                      {headerMap?.get(col) ?? formatColumnHeader(col)}
