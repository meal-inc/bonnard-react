...
export function applyFormat(value: unknown, fmt: string): string {
  ...
  try {
    ...
    // Invalid format pattern — fall back to raw value
  }
...
export function parseFmtProp(fmt: string): Map<string, string> {
  ...
  // Split on commas followed by column_name:format boundary
  // Column names may be dot-qualified (e.g. "sales_performance.total_revenue")
