...
import type { ParsedDashboard, DashboardSection, ComponentTag, SeriesType, ColumnConfig } from '../lib/types';
...
/** BigValue grids use auto-fit with a max so cards don't stretch full width */
const BIG_VALUE_COL_STYLES: Record<string, React.CSSProperties> = {
  '1': { gridTemplateColumns: 'repeat(1, minmax(min-content, 280px))' },
  '2': { gridTemplateColumns: 'repeat(2, minmax(min-content, 280px))' },
  '3': { gridTemplateColumns: 'repeat(3, minmax(min-content, 280px))' },
  '4': { gridTemplateColumns: 'repeat(4, minmax(min-content, 280px))' },
};
...
function groupConsecutiveBigValues(sections: DashboardSection[]): DashboardSection[] {
  ...
  const flushRun = () => {
    if (bigValueRun.length > 0) {
      result.push({
        kind: 'grid',
        props: { cols: String(Math.min(bigValueRun.length, 4)), _bigvalue: 'true' },
...
function SectionRenderer({
  ...
  if (section.kind === 'grid') {
    ...
    const isBigValueGrid = section.props._bigvalue === 'true';
    const colStyle = isBigValueGrid
      ? (BIG_VALUE_COL_STYLES[cols] || BIG_VALUE_COL_STYLES['4'])
      : (GRID_COL_STYLES[cols] || GRID_COL_STYLES['2']);
    return (
  ...
  // Wrap chart components in a card — BigValue and DataTable have their own borders
  const skipCard = section.component.type === 'BigValue' || section.component.type === 'DataTable';
  const inner = <ComponentRenderer component={section.component} queryStates={queryStates} />;

  if (skipCard) return inner;
    ...
    >
      {(data) => {
        switch (type) {
          ...
          case 'DataTable': {
            const columnConfigs: ColumnConfig[] | undefined = props._columns
              ? JSON.parse(props._columns)
              : undefined;
            return (
              <DataTable
                data={data}
