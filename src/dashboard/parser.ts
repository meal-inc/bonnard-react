...
/**
 * Regex to match self-closing Column tags inside a DataTable wrapping body:
 *   <Column field="orders.revenue" header="Revenue" fmt="usd2" />
 *
 * Captures: [1] attributes string
 */
const COLUMN_TAG_RE = /<Column\s+([\s\S]*?)\/>/g;
...
function splitIntoSections(html: string): { sections: DashboardSection[]; inputs: ComponentTag[] } {
  ...
  // Combined regex: match Grid wrapping tags, DataTable wrapping tags, OR self-closing component tags (including inputs)
  // Groups: [1] Grid attrs, [2] Grid inner, [3] DataTable attrs, [4] DataTable inner,
  //         [5] self-closing component name, [6] self-closing component attrs
  const COMBINED_RE = /<Grid\s+([\s\S]*?)>([\s\S]*?)<\/Grid>|<DataTable\s+([\s\S]*?)>([\s\S]*?)<\/DataTable>|<(BigValue|LineChart|BarChart|AreaChart|DataTable|PieChart|DateRange|Dropdown)\s+([\s\S]*?)\/>/g;
  ...
  while ((match = COMBINED_RE.exec(html)) !== null) {
    ...
    if (match[1] !== undefined) {
      ...
      // Wrapping DataTable match: [3] = attrs, [4] = inner content with <Column> tags
      const props = parseProps(match[3]);
      const columnConfigs = parseColumnChildren(match[4]);
      if (columnConfigs.length > 0) {
        props._columns = JSON.stringify(columnConfigs);
      }
      const component: ComponentTag = { type: 'DataTable', props };
      sections.push({ kind: 'component', component });
    } else {
      // Component match: [5] = name, [6] = attrs
      const component = parseComponentTag(match[5], match[6]);
      if (component) {
...
/**
 * Parse <Column /> children inside a wrapping <DataTable>...</DataTable> body.
 * Returns an array of ColumnConfig objects preserving declaration order.
 */
function parseColumnChildren(innerHtml: string): ColumnConfig[] {
  const configs: ColumnConfig[] = [];
  COLUMN_TAG_RE.lastIndex = 0;

  let colMatch: RegExpExecArray | null;
  while ((colMatch = COLUMN_TAG_RE.exec(innerHtml)) !== null) {
    const props = parseProps(colMatch[1]);
    if (props.field) {
      const config: ColumnConfig = { field: props.field };
      if (props.header) config.header = props.header;
      if (props.fmt) config.fmt = props.fmt;
      configs.push(config);
    }
  }
